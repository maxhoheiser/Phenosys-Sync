...
%            output_data_ttl((end+1),1:17) = output_data(i,18:34);
...
output_name_error(1,2:9) = output_name_vec_time;
output_name_error(1,10:18) = output_name_vec_event;
...
%                 i
%                 if test_fit(ttl_corrected(i,5),ground_truth(j,3),k) && ttl_corrected(i,2)==1 && ground_truth(j,4) == 0 && ground_truth_vec(i,1)==0 && abs(ground_truth(j,2)-ttl_corrected(i,4))<=x
...
for k = 1:30
    for j = 7:length(ground_truth)
        check_vec=ground_truth(j:-1:(j-6),4);
        for i = 1:length(ttl_corrected)
            if test_fit(ttl_corrected(i,5),ground_truth(j,3),k) && ttl_corrected(i,2)==1 && ground_truth(j,4) == 0 && sum(ttl_corrected(i,5)==check_vec)==0
                ground_truth(j,4) = ttl_corrected(i,5);
                ground_truth(j,5) = ttl_corrected(i,4);
...
% ttl_not_in_excel=[];
% for i = ground_truth_vec
%     ttl_not_in_excel(end+1,:)=ttl_corrected(i,:);
% end
...
output_name = ["excel-time", "excel-behavior", "excel-tim-rel", "ttl-time-rel", "ttl-behavior", "ttl-dur", "ttl-0/1","ttl-time-true", "time-diff", "behavior-dif", "num"];
output_file_ground_truth = [output_name; ground_truth];
filename = 'output_file.xlsx';
writematrix(output_file_ground_truth,filename,'Sheet', 'Details');
%% find ttl trials not in excel
...
for i = 1:size(output_data,1)
    ...
    for j = 23
        for k = 1:size(ground_truth,1)
            if output_data(i,j) == ground_truth(k,8)
                count = 1;           
            end
        end
        if count ~= 1
            not_in_excel(end+1,1:37) = output_data(i,1:37); 
            count = 0;
        end 
    end
end
...
% output_file_not_in_excel = [filename; not_in_excel];
% filename = 'output_file.xlsx';
% writematrix(output_file_not_in_excel,filename,'Sheet', 'Details');
