...
compare_2_methods(compare_2_methods(:,14)>40,14)=0;

j=1;
for i = [1 2 3 4 10 11]
    col = ["r","g","b","m","c","k"]; 
    vec = compare_2_methods(compare_2_methods(:,4)==i,[13 14]);
    scatter(vec(:,1),vec(:,2),'.',col(j));
    j = j+1;
    hold on
end

vec = compare_2_methods(not(ismember(compare_2_methods(:,4),[1 2 3 4 10 11])),[13 14]);
scatter(vec(:,1),vec(:,2),'*','k');
